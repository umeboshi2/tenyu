...
from trumpet.resources import ResourceObject
from trumpet.models.usergroup import User


from tenyu.views.util import check_login_form


def make_page(appname, settings, basecolor=None):
    template = 'tenyu:templates/mainview.mako'
...
def forbidden_view(exc, request):
    APIROOT = request.registry.settings.get('default.apiroot', '/rest/v0')
    if request.path.startswith(APIROOT):
        return Response(json=dict(status=403, result='failed'))
    root = ResourceObject('')
    login = ResourceObject('login', root)
